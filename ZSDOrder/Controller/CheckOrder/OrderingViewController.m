...
@interface OrderingViewController ()<UITableViewDelegate, UITableViewDataSource, CheckOrderServiceDelegate, OrderDetailServiceDelegate, AuditServiceDelegate>
...
- (void)viewDidLoad {
    ...
    if([_app.user.USER_TYPE isEqualToString:kMANAGER] || [_app.user.USER_TYPE isEqualToString:kALL]) {
...
- (void)scrollViewDidEndDeceleratingMethod {
    ...
    NSLog(@"scrollViewDidEndDeceleratingMethod");
    [self viewDidAppear];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(_myTableView.editing) {
        ...
        // 已确认才能编辑
        if([order.ORD_WORKFLOW isEqualToString:@"已确认"]) {
            ...
            [Tools showAlert:self.view andTitle:@"此订单未确认"];
        }
    }
